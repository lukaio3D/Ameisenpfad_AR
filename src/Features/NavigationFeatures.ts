...
export default async function createNavigationFeatures(scene: Scene, agentMesh: TransformNode) {
  // Navmesh Setup
  ...
  let matdebug = new StandardMaterial("matdebug", scene);
  ...
  let crowd = navigationPlugin.createCrowd(10, 1, scene);
  let agents = [];
  let i;
  let agentParams = {
    radius: 0.5,
