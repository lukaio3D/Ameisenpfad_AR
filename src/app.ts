...
class App {
    ...
    async initialize() {
        ...
        let hitTest;
        let modelPlaced = false; // Variable, um zu verfolgen, ob das Modell platziert wurde

        b.isVisible = false;

        (xrTest as WebXRHitTest).onHitTestResultObservable.add((results) => {
            if (results.length && !modelPlaced) { // Nur aktualisieren, wenn das Modell noch nicht platziert wurde
        ...
        scene.onPointerDown = async (evt, pickInfo) => {
            if (hitTest && anchors && xr.baseExperience.state === WebXRState.IN_XR) {
                await (anchors as WebXRAnchorSystem).addAnchorPointUsingHitTestResultAsync(hitTest);
                modelPlaced = true; // Modell als platziert markieren
                b.isVisible = true; // Modell sichtbar machen
                b.position.copyFrom(marker.position); // Position des Modells auf die des Markers setzen
                b.rotationQuaternion.copyFrom(marker.rotationQuaternion); // Rotation des Modells auf die des Markers setzen
                marker.isVisible = false; // Marker ausblenden
            }
        };
